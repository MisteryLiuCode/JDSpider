...
for book_page in range(1,100,2):
    # 指定网址
    url_json="https://search.jd.com/Search?keyword=教育&enc=utf-8&qrst=1&rt=1&stop=1&vt=2&wq=Python&page={0}&s=54&click=0".format(book_page)
    ...
    for soup_li in soup_allli:
        ...
        # bookName=soup_name.get_text().rstrip('\n')
        print("书名：",soup_name.get_text().replace('(','').replace(')',''),file=spider_lzl)
        print("价格：",soup_price.get_text(),file=spider_lzl)
        test1=soup_author.get_text().rstrip('著')
        test=soup_author.get_text().replace('著','').replace('\t','').replace('\n','').replace('|','')
        try:
            print("作者：",soup_author.get_text().replace('著','').replace('\t','').replace('\n','').replace('|','').replace(' ',' ').replace('(','').replace(')','').replace('（','').replace('）',''),file=spider_lzl)
        except:
